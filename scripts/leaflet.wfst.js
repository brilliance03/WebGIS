...
L.WFST = L.GeoJSON.extend({
    ...
    */
    _loadExistingFeatures: function(){
		...
		var geoJsonUrl = this.options.url + '&outputFormat=application/json';
        this._ajax({
            url: geoJsonUrl,
            success: function(res){
                res = JSON.parse(res);
    ...
    */
    _loadFeatureDescription: function(){
        ...
        //TODO:由于源代码作者仅针对geoserver开发，所以mapserver要使用的话，请求的url不符合要求，还要再做修改
        //TODO:这里暂时写死url，测试通过之后再修改
        var describeFeatureUrl = "http://localhost/cgi-bin/mapserv.exe?map=mapserver/mapserver_china.map&version=1.0.0&maxfeatures=50&typename=provinces_simplify&service=WFS&version=1.0.0&request=DescribeFeatureType"
        this._ajax({
